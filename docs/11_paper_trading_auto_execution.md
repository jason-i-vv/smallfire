...
### 降低自动交易阈值解决冷启动问题 (2026-04-18)

**问题**: 评分机制存在冷启动死循环：
- 评分依赖历史胜率统计（signal_type_stats）
- 新系统没有历史交易 → 胜率为0 → 评分偏低（38分左右）
- 评分 < 60（自动交易阈值）→ 不触发交易
- 没有新交易 → 永远没有历史数据

**改动**:
- **config/config.yml**: `auto_trade_score_threshold` 从 60 降到 45
- 评分 >= 45 即可触发自动交易（与创建阈值一致）
- 短期内以积累数据为主，不以取得收益为主

**计算示例**（单一信号、强度2、无历史数据）：
| 维度 | 得分 |
|------|------|
| StrategyWinRate | 50 × 30% = 15 |
| MultiConfluence | 10 × 25% = 2.5 |
| SignalStrength | 40 × 20% = 8 |
| VolumeConfirm | 50 × 15% = 7.5 |
| MarketRegime | 50 × 10% = 5 |
| **总分** | **38 分** |

原阈值 60 会导致 38 分的机会无法触发交易，形成死循环。降低到 45 后可正常触发。

### 移除同标持仓限制，支持同标的多个仓位 (2026-04-18)

**问题**: 自动交易会检查同标的是否已有持仓，如有则跳过。但数据收集场景下应该允许同标的多个仓位。

**改动**:
- **auto_trader.go**: 移除 `GetOpenBySymbol` 检查，不再限制同标只能有一个仓位
- 每次机会都会创建新的交易记录，方便收集更多数据
