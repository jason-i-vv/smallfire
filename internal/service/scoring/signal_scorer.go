...
type ScoringWeights struct {
	StrategyWinRate float64 `mapstructure:"strategy_win_rate" json:"strategy_win_rate"` // 策略历史胜率 30%
	MultiConfluence float64 `mapstructure:"multi_confluence" json:"multi_confluence"`   // 多策略共识   25%
	SignalStrength  float64 `mapstructure:"signal_strength" json:"signal_strength"`     // 信号强度     20%
...
func (s *SignalScorer) calcMultiConfluence(confluenceCount, totalSignals int, ratio float64) int {
	...
	return clamp(int(baseScore+countBonus), 0, 100)
}
...
func (s *SignalScorer) calcVolumeConfirm(volumeRatio float64) int {
	...
	// volumeRatio: 0.5x → 25分，1.0x → 50分，2.0x → 100分
